...
export const connectSocket = (userId, setCallerId, setModalVisible, setCallAccepted, incomingOfferRef, setCallType) => {
    if (socket)
        return;
...
export const startCall = (receiverId, senderId, callType) => {
    if (!socket)
        return;
...
export const acceptCall = (offer, callerId, callType) => {
    peerConnection = new RTCPeerConnection(rtcConfig);
    navigator.mediaDevices
        .getUserMedia({ video: callType === "video", audio: true })
        .then((stream) => {
