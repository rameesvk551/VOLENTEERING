...
export const loadMessages = createAsyncThunk("messages/loadMessages", async ({ userId, receiverId }, { rejectWithValue }) => {
    try {
...
export const sendMessage = createAsyncThunk("messages/sendMessage", async ({ content, receiverId }, { rejectWithValue }) => {
    try {
        const res = await axios.post(`${server}/message/send-message/${receiverId}`, { content, receiverId }, { withCredentials: true });
        return res.data;
    }
    catch (err) {
        return rejectWithValue(err.response?.data || err.message);
